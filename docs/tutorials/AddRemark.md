...
Create a `RemarkCommandParser` class in the `seedu.insurancepal.logic.parser` package. The class must extend the `Parser` interface.
...
Create a new `Remark` in `seedu.insurancepal.model.client`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Simply add the following to [`seedu.insurancepal.ui.ClientCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-0c6b6abcfac8c205e075294f25e851fe).
